...
# 진짜 출력값과 비교함. 
# 이 비교는 문자열 == 비교라서 애매함이 없음 — 창의성이나 스타일 판단이 필요한 게 아니라 "같다/다르다"만 확인하는 기계적인 작업.
#
...
def extract_code_block(question_text: str) -> Optional[str]:
    """
    문제 텍스트 안의 코드 블록을 최대한 관대하게 추출한다.

    AI는 코드 펜스를 여러 변형으로 뱉는다:
``````python\n코드```      (언어명 + 바로 줄바꿈)
`````py 코드```           (짧은 언어명)
````\npython\n코드```    (펜스 다음 줄에 언어명이 따로)
``` 코드 ```            (언어명 없음)
    기존 정규식은 '펜스 + 언어명 + 공백 + 반드시 \n'을 요구해서,
    '펜스 바로 뒤 줄바꿈' 같은 흔한 형태를 놓치고 None을 반환했다.
    (이게 온보딩 퀴즈가 전부 검증 불가(None)로 떨어진 원인)

    수정 방침:
        1) 여는 펜스 뒤의 '언어명 한 줄'을 선택적으로 소비 (있든 없든 OK)
        2) 그 뒤 모든 내용을 닫는 펜스까지 코드로 캡처
        3) 펜스가 아예 없으면(마크다운 없이 평문 코드) 마지막 폴백으로
        'print(' 등 코드 흔적이 있으면 텍스트 전체를 코드로 시도
    """
    if not question_text:
        return None

    # 1) 표준 펜스 형태: ```[언어명]? ... ```
    #    - [ \t]*  : 펜스와 언어명 사이 공백/탭 (줄바꿈 아님)
    #    - [a-zA-Z]* : 언어명(python, py, 등) — 없어도 됨
    #    - \s*     : 언어명 뒤 공백/줄바꿈 (여기서 줄바꿈을 흡수)
    #    - (.*?)   : 코드 본문 (비탐욕, 닫는 펜스 전까지)
    match = re.search(
        r"```[ \t]*[a-zA-Z]*\s*(.*?)```",
        question_text,
    ...
    # 2) 폴백: 펜스가 전혀 없지만 코드처럼 보이는 경우
    #    (AI가 마크다운 없이 평문으로 코드를 넣은 케이스)
    #    print(가 있으면 실행 가능한 코드로 간주하고 통째로 시도
    if "print(" in question_text:
