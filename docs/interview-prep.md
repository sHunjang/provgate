...
## Q. 최종 제출 시 보안을 어떻게 처리했나요?
게이트 통과 시 발급된 토큰을 최종 제출 시 검증합니다.
토큰은 secrets.token_urlsafe(32)로 생성해서 위조가 불가능하고,
24시간 만료 시간과 단일 사용(used=TRUE) 처리로
토큰 재사용을 방지했습니다.
토큰이 없거나 만료됐거나 이미 사용된 경우 403 에러를 반환합니다.

## Q. 전체 학습 플로우를 설명해주세요.
1. 온보딩: 수준 선택 → 진단 퀴즈 → 수준 확정
2. 문제 풀기: 코드 에디터에서 Python 코드 작성
3. 테스트: Pyodide로 브라우저에서 직접 실행 및 채점
4. AI 힌트: 막히면 소크라테스식 힌트 최대 3단계
5. 게이트: 테스트 통과 시 같은 개념 다른 문제로 이해도 검증
6. 제출: 토큰 검증 후 최종 제출
7. 피드백: 힌트 사용 횟수, 게이트 시도 횟수, 소요 시간 + 유사 문제 추천