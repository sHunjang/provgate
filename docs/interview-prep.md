...
## Q. 동적 라우팅을 어떻게 구현했나요?
Next.js App Router의 파일 시스템 기반 동적 라우팅을 활용했습니다.
app/problems/[id]/page.tsx 파일을 만들면
/problems/1, /problems/2 등 모든 id에 대해 같은 컴포넌트가 렌더링됩니다.
useParams() 훅으로 URL의 id 값을 읽어서 해당 문제를 API로 조회했습니다.

## Q. 힌트 시스템을 어떻게 설계했나요?
힌트는 DB에 hint_1, hint_2, hint_3 컬럼으로 저장했습니다.
프론트엔드에서 hintStep 상태로 현재 힌트 단계를 관리하고
버튼 클릭 시 단계가 올라가는 방식으로 구현했습니다.
힌트 사용 횟수는 나중에 submissions 테이블에 기록해서
학습 분석 데이터로 활용할 예정입니다.

## Q. 소크라테스식 힌트를 어떻게 구현했나요?
Claude API의 system 프롬프트에 "정답을 직접 알려주지 말고
질문으로 유도하라"는 규칙을 명시했습니다.
힌트 단계(1~3)에 따라 구체성을 조절했습니다.
1단계는 방향만 제시하고, 2단계는 개념을 언급하고,
3단계는 사용할 함수나 문법을 알려주되 완성 코드는 절대 보여주지 않습니다.
사용자의 현재 코드를 프롬프트에 포함시켜
어디서 막혔는지 파악한 맞춤형 힌트를 제공합니다.

## Q. 이해 확인 게이트를 어떻게 구현했나요?
테스트 케이스를 모두 통과하면 게이트 모달이 자동으로 팝업됩니다.
Claude API로 같은 개념의 다른 유형 문제를 동적 생성하고,
정답을 맞추면 secrets.token_urlsafe(32)로 암호학적으로
안전한 토큰을 발급합니다.
토큰은 gate_tokens 테이블에 저장되고 24시간 후 만료됩니다.
최종 제출 시 토큰 유효성을 검증해서 게이트를 우회할 수 없도록 했습니다.
최대 3회 재시도가 가능하고 모두 실패하면 문제를 다시 풀어야 합니다.

## Q. 토큰을 왜 secrets 모듈로 생성했나요?
Python의 random 모듈은 예측 가능한 난수를 생성해서
보안에 취약합니다.
secrets 모듈은 암호학적으로 안전한 난수를 생성해서
토큰 위조가 불가능합니다.
token_urlsafe(32)는 URL에 안전한 문자만 사용하는
256비트 랜덤 토큰을 생성합니다.