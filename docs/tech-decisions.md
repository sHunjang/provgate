...
## 문제 데이터 설계
- 고정 문제: DB에 직접 저장 (검증된 문제, 정확한 테스트 케이스)
- test_cases: JSONB 타입으로 저장 (가변적인 입출력 구조)
- hint_1/2/3: 단계별 힌트를 컬럼으로 분리
  이유: 힌트 사용 단계를 추적하고 순서를 보장하기 위함
- starter_code: 초기 코드 템플릿 제공
  이유: 학습자가 코드 구조 파악에 시간을 낭비하지 않도록